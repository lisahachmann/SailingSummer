...
#Translated into Python 2.7 for the Team Sailing Project, as a test before implementing the Keypad Pi Client-Server code"""
class UDP_RX(object):
...
#While waiting for a message UDP_RX to be started or to send another message, UDP_TX will print a line of "."s one every 2 seonds."""
    ...
    def __init__(self,IP="192.168.17.142",port=5280):
        ...
        while True:
            try:
                ...
                byte_message = bytearray(decoded, encoding="UTF-8")
                byte_sending = sock.sendto(byte_message, source_address)
            except timeout: 
                #flush= True            
                print "."#,end==""  # if process times out, just print a "dot" and continue waiting.  The effect is to have the server print  a line of dots
