...
#The server, runing UDP_RX, determines the client's port number from each message it receives.
    ...
    def __init__(self,Server_Address=("192.168.17.142",5280)):   # create a socket instance.
        ...
        sock= socket(AF_INET,SOCK_DGRAM)  # open the socket
        ...
        while True:
            ...
            try:
                bytearray_msg, Server_Address = sock.recvfrom(1024) # 1024 is the buffer length
                                                                 # allocated for receiving the
                                                                 # datagram (i.e., the packet)
                                                                 
                server_IP, server_port = Server_Address    # the source iaddress is ("127.0.0.1",client port number)
